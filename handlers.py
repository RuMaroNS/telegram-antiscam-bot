...
async def process_rating_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        if ADMIN_ID:
            ...
            await bot.send_message(ADMIN_ID, f"👑 Админ, новая жалоба на @{data['target_user_str']}!", reply_markup=kb)
...
async def check_user_in_db(message: Message, state: FSMContext, bot: Bot):
    ...
    if scammer:
        ...
        await message.answer(f"✅ Пользователь @{search_username} не найден в базе данных скамеров.")

# =====================================================================
# ПАНЕЛЬ МОДЕРАТОРА (АДМИНКА БЕЗ СЛОЖНОЙ РАЗМЕТКИ)
# =====================================================================
...
async def admin_view_requests(event):
    ...
    # Убрали всю хрупкую разметку звезд и подчеркиваний, теперь бот никогда не упадет
    text = (
        f"📋 Заявка #{req_id}\n\n"
...
async def handle_decision(callback: CallbackQuery):
    ...
    if action == "approve" and current_req:
        ...
        await callback.message.answer(f"✅ Заявка #{req_id} одобрена. Данные внесены.")
    else:
